...
int stimFreq1 = 5000;
int rewDur = 100;
int postRewDelay = 2000;

// 
String programName = "jackLickTrain_032917a.ino";
...
//  portTone2.begin(spkrPin2);
  ...
  Serial.println("postRewDelay=" + String(postRewDelay));
...
void loop() {
  ...
  if (lickVal1==1) {
    ...
    Serial.println("REWARD, lickPort=1, millis=" + String(rewTime));
    delay(postRewDelay);
  }
  else if (lickVal2==1) {
    rewTime = millis();
    ...
    Serial.println("REWARD, lickPort=2, millis=" + String(rewTime));
    delay(postRewDelay);
  }

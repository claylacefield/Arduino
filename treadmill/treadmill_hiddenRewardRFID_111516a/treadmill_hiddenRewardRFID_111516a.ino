...
NOTE: 
This version is for Arduino DUE

1. Look for rotary encoder input
2. update position
3. see if position is in reward zone
4. if so, look for licks
5. if lick in reward zone, give reward
6. reset position at RFID lap counter

RFID code based upon Bildr RFID example
...
void serialEvent() {
  ...
  currPos = 0;  // reset the position counter at lap boundary
}

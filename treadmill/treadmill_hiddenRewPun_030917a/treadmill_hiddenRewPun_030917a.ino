...
// output to context arduino
int ctxtArdPin = 46;
int ctxtArdOn = 0;

// Other pins
int lickPin = 49;
int spkrPin = 47;  // NOTE: Tone doesn't work on DUE
int ledPin = 13;
...
void checkPun() {
  ...
  if (numPulses<6) { 
    ...
    numPulses = 0;
    if (prevPun==2) {
      digitalWrite(ctxtArdPin, HIGH);
      delay(5);
      digitalWrite(ctxtArdPin, LOW);
      Serial.println("contextOff, millis=" + String(millis()));
    }
    prevPun = 0;
  }
}
...
void serialEvent2() {
  ...
  if(Serial1.available() > 0) {
    ...
    if (ind == 12) {                          // if 12 digit read is complete
      ...
      else if (isCalibrated == 2 && ctxtArdOn == 0) {
        digitalWrite(ctxtArdPin, HIGH);
        Serial.println("contextOn, millis=" + String(millis()));
        ctxtArdOn = 1;
        delay(5);  // not ideal to use delay but hopefully this is too short to cause problems
        digitalWrite(ctxtArdPin, LOW);
      }

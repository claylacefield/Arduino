...
String programName = "treadmill_hiddenRewPun_030617a";
int rewBegPos = 1000;
int rewEndPos = 2000;
int rewDur = 500;
...
// pun vars
int punTime = punTimeMin*60*1000;
int prevPun = 0;
...
//    }
    ...
    if (millis()-startTime > punTime && prevPun == 0) {
      prevPun = 1; 
      givePun();
    }
...
void checkPun() {
  ...
  if (currentMillis-punStartTime<punDur){ //  && punOn == 1) {
  
    if (currentMillis - prevMillis2 >= punInterval) {
      ...
      digitalWrite(solPin2, solState2);
    }
  }
